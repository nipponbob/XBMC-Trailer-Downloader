...
from ConfigClass import config_class
...
#
    debug_info_payload = time.strftime("%d/%m/%Y ") + time.strftime("%H:%M:%S - ") + debug_info 
    if config.log_file:
        with open("debug.log", "w") as f:       
            f.write(debug_info_payload + '\n')  
    if config.verbose_output : print(debug_info_payload)   # Print to the console if required
...
#    
    ...
    days_old = config.file_age * seconds_in_day # The desired delete time in seconds
    del_time = time.time() - days_old           # Get the current time - file age time
    for filename in os.listdir(config.save_path):
        path_to_file = os.path.join(config.save_path, filename)
...
#   
    ...
    if config.multi_part :                              # Download multiple clips for the same movie
        current_movie = active_link.split('/')[-1]      # This gets the filename from the url
        if last_down == current_movie : 
            return 
        grab_type = '-tlr'
        if config.site_pref in active_link and config.quality in active_link and config.grab_type in active_link: 
    ...
    if not config.multi_part:                           # Download 1 trailer for each movie
        current_movie = active_link.split('/')[-2]      # This gets the movie name from the url
        grab_type = '-tlr'       
        if last_down == current_movie :                 
            return 
        if config.site_pref in active_link and config.quality in active_link and  grab_type in active_link :
            writeDebug("Found Trailer : " + active_link)                                           
    ...
    if config.download_clips :          # Download clips
        grab_type = 'clip', '-fte1' 
        for grab in grab_type :
            if config.site_pref in active_link and config.quality in active_link and grab in active_link: 
...
#           before searching.
    ...
    if config.custom_label != '': 
        search_str = extracted_filename[0] + config.custom_label + extracted_filename[1]  # Add the custom string to the search_str
    elif config.custom_label == '': 
        search_str = extracted_filename[0] + extracted_filename[1]                   # If there is no custom string   
...
#  
    rename_count = 0
    for filename in os.listdir(config.save_path):      
        if not config.custom_label in filename and not 'clip' in filename and not '-fte' in filename:  # Check that custom_string and clip are not in filename
             if filename.endswith(config.filetype_dl):                         # File ext to search for
                 path = os.path.join(config.save_path, filename)            # Location + filename   
                 extracted_filename = extractFilename(filename)
...
#           add boolean value to parameters to control wethr this checks a new link or returns the value!
    ...
    try:
        new_url = requests.get(config.base_url + new_link)   # make a new url to scrape new page                
    except requests.exceptions.ConnectionError as error_msg:
        writeDebug(str(error_msg))
        return
    data = new_url.text
...
if __name__ == '__main__':
    ...
    if not os.path.isfile(config_file):
        print "Config file missing. Please download again"    
        quit()
    config = config_class.ConfigStruct()
    dl_clip_count = 0
    dl_trailer_count = 0
    last_down = ''
    ...
    checkDirectory(config.save_path)                                  # Check if path exists, create it if it doesn't   
    print "XBMC Trailer Downloader"
    print "Current Version - %s" %config.current_version
    if config.download_latest   : makeSoup(config.base_url + '/page/1/')
    if config.most_watched      : makeSoup(config.base_url + '/most-watched')
    if config.top_movies       : makeSoup(config.base_url + '/top-movies/')
    if config.opening_this_week : makeSoup(config.base_url + '/opening-this-week/')
    if config.coming_soon  : makeSoup(config.base_url + '/coming-soon/')
    writeDebug(str(dl_trailer_count) + ' trailers downloaded.')    
    writeDebug(str(dl_clip_count) + ' clips downloaded.')
    if config.del_files : deleteFiles()
    if config.rename_trailer : renameTrailers() 
